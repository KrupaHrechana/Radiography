...
myRadiographia <- raster("D:\\github\\Radiography\\Radiography\\Particles_3h.tif")
...
time <- 0.25                        # Select exposition period in hours (0.5, 1, 7.5, 16, 21, 24, 48, 72, 96)
working_name <- "Particles_15min"
dpi <- 508
resize_pixels <- 10
fon_level <- 36
detect_level_for_part <- 52
focal_filter <- 9
max_local_filter <- 20
sample_weigth_g <- 4.5
...
if (time==0.25) {
   a0 <- -1.666E-09
   a1 <- 0.00348
   print ("Ok! You selected 15 minutes")
} else if (time==0.5) {
   a0 <- -2.454E-09
   a1 <- 0.00398
...
writeRaster(r2, "D:\\github\\Radiography\\Radiography\\pre_Particles_3h.tif", dataType = "INT2U")
...
writeOGR(ttops, "D:\\github\\Radiography\\Radiography\\detect_paticals", working_name, driver = "ESRI Shapefile")

# Save a set of tree crown polygons
writeOGR(polygon, "D:\\github\\Radiography\\Radiography\\output_data", working_name, driver = "ESRI Shapefile")

# Add a second data set in a new worksheet
write_xlsx(polygon@data,"D:\\github\\Radiography\\Radiographys\\Particles_3h.xlsx")
